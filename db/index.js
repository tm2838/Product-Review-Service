...
const getReviewMeta = (productId) => {
  const reviewMeta = { product_id: productId };
  const charRating = {};

  return pool.query(`SELECT * FROM reviews where product_id=${productId}`)
    .then((reviewsRaw) => {
      const reviews = reviewsRaw.rows;
          ...
          pool.query(`SELECT value FROM characteristic_reviews where characteristic_id=${char.id}`)
            .then((results) => {
              ...
              if (results.rows.length) {
                charRating[char.name].value
                  += results.rows.reduce((curr, accu) => (
                    { value: accu.value + curr.value }), { value: 0 }).value / results.rows.length;
              }
            }),
        );
      });
...
const addReview = (review) => {
  const {
    ...
    product_id, rating, summary, body, name, email, recommend, photos, characteristics,
  } = review;
  let reviewId;
